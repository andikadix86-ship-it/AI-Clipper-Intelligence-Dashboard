...
const platformOptions: Array<{ value: IntelligencePlatform; label: string }> = [
  ...
  { value: "GOOGLE_TRENDS", label: "Google Trends Demo" },
  { value: "REDDIT", label: "Reddit OAuth" },
  { value: "TIKTOK", label: "TikTok Demo" },
  { value: "SHOPEE", label: "Shopee Demo" },
  { value: "TOKOPEDIA", label: "Tokopedia Demo" }
];

export function IntelligenceSearchPanel({ mode }: { mode: IntelligenceMode }) {
  const [form, setForm] = useState({ keyword: "AI tools", niche: "", region: "ID", language: "id", timeRange: "7d", platforms: ["YOUTUBE", "GOOGLE_TRENDS"] as IntelligencePlatform[] });
  ...
  const [message, setMessage] = useState("Search dilakukan hanya saat tombol ditekan. Cache 30 menit menghemat quota YouTube.");
  ...
  async function search(refresh = false) {
    ...
    try {
      ...
      setResults([]);
      setMessage(error instanceof Error ? error.message : "Intelligence search gagal.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <section className="rounded-2xl border border-white/[0.06] bg-[#111A2E] p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Search public signals, compare source transparency, then analyze one opportunity with a deterministic framework.</p>
        </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-2">
        {results.map((result) => <article key={result.id} className="rounded-xl border border-white/[0.06] bg-white/[0.025] p-4">
          <div className="flex flex-wrap items-center gap-2"><Badge tone={result.isDemo ? "warning" : "success"}>{result.isDemo ? "Demo Source" : "Real Public Data"}</Badge><Badge tone="default">{result.platform}</Badge><Badge tone="default">{result.trendDirection}</Badge></div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {result.sourceUrl ? <a href={result.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg border border-white/[0.08] px-3 py-2 text-xs font-semibold text-slate-200"><ExternalLink className="h-3.5 w-3.5" />Source</a> : null}
          </div>
        </article>)}
      </div>
