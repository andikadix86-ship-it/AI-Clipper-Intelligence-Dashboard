...
type Tab = "hooks" | "scripts" | "captions" | "hashtags" | "ctas" | "videoPrompts";
...
export function AffiliateContentFactory({ campaign }: { campaign: AffiliateCampaignDraft }) {
  ...
  const studioContext = useMemo(() => {
    const context = productStudioContext({
      ...
      platform: campaign.platform as "Shopee" | "TikTok Shop" | "Tokopedia",
      ...
      <div className="mt-4 space-y-3">
        {kit[activeTab].map((value, index) => (
          <div key={`${activeTab}-${index}`} className="rounded-xl border border-white/10 bg-white/[0.04] p-3">
            <textarea value={value} onChange={(event) => updateResult(index, event.target.value)} rows={activeTab === "scripts" || activeTab === "videoPrompts" ? 4 : 2} className="premium-input px-3 py-2 text-sm" />
