...
const emptyTelegram: TelegramSettingDto = { botTokenMasked: "", chatId: "", status: "NOT_CONFIGURED", statusLabel: "Not configured" };
const emptyGoogleOAuth: GoogleOAuthSettingDto = { clientIdMasked: "", clientSecretMasked: "", redirectUri: "", status: "NOT_CONNECTED", statusLabel: "Not configured" };
...
export default function SettingsPage() {
  ...
  const [telegram, setTelegram] = useState<TelegramSettingDto>(emptyTelegram);
  ...
  const [googleOAuth, setGoogleOAuth] = useState<GoogleOAuthSettingDto>(emptyGoogleOAuth);
  ...
  useEffect(() => {
    ...
    fetch("/api/telegram/settings")
      ...
      .then((data) => {
        const setting = normalizeTelegram(data?.telegram ?? data?.setting);
        setTelegram(setting);
        setTelegramForm((current) => ({ ...current, chatId: setting.chatId }));
      })
      .catch(() => {
        setTelegram(emptyTelegram);
        setMessage("Telegram settings gagal dimuat. Dashboard approval tetap tersedia.");
      });
    fetch("/api/settings/google-oauth")
      .then((response) => response.json())
      .then((data) => {
        const setting = normalizeGoogleOAuth(data?.setting);
        setGoogleOAuth(setting);
        setGoogleForm((current) => ({ ...current, redirectUri: setting.redirectUri }));
      })
      .catch(() => {
        setGoogleOAuth(emptyGoogleOAuth);
        setMessage("Google OAuth settings gagal dimuat.");
      });
    fetch("/api/settings/oauth-providers")
      .then((response) => response.json())
      .then((data) => {
        const settings = normalizeOAuthSettings(data?.settings);
        setOauthSettings(settings);
        setOauthForms((current) => ({
          ...current,
          TIKTOK: { ...current.TIKTOK, redirectUri: settings.find((item) => item.provider === "TIKTOK")?.redirectUri ?? "" },
          META: { ...current.META, redirectUri: settings.find((item) => item.provider === "META")?.redirectUri ?? "" }
        }));
      })
  ...
  async function loadIntelligenceProviders() {
    try {
      ...
      setIntelligenceProviders(normalizeArray<IntelligenceProviderSetting>(data?.settings));
    } catch {
  ...
  async function loadProviders() {
    try {
      ...
      const data = await response.json();
      setProviders(normalizeArray<AIProviderDto>(data?.providers));
    } catch {
      setProviders([]);
    }
  }
  ...
  async function loadProviderStatus() {
    try {
      ...
      const data = await response.json();
      setProviderStatuses(normalizeArray<ProviderStatusRow>(data?.providers));
    } catch {
      setProviderStatuses([]);
    }
  }
  ...
  async function runProviderTest(target: TestTarget) {
    ...
    try {
      ...
      if (target === "telegram") setTelegram(normalizeTelegram(data?.telegram ?? data?.setting));
      await Promise.all([loadProviders(), loadProviderStatus()]);
    } catch (error) {
  ...
  async function saveTelegram() {
    ...
    try {
      ...
      const setting = normalizeTelegram(data?.telegram ?? data?.setting);
      setTelegram(setting);
      setTelegramForm({ botToken: "", chatId: setting.chatId });
  ...
  async function testTelegram() {
    ...
    try {
      ...
      setTelegram(normalizeTelegram(data?.telegram ?? data?.setting));
  ...
  async function saveGoogleOAuth() {
    ...
    try {
      ...
      const setting = normalizeGoogleOAuth(data?.setting);
      setGoogleOAuth(setting);
      setGoogleForm({ clientId: "", clientSecret: "", redirectUri: setting.redirectUri });
  ...
  async function saveOAuthProvider(provider: "TIKTOK" | "META") {
    ...
    try {
      ...
      const setting = normalizeOAuthSetting(data?.setting, provider);
      setOauthSettings((current) => {
        const exists = current.some((item) => item.provider === provider);
        return exists ? current.map((item) => item.provider === provider ? setting : item) : [...current, setting];
      });
      setOauthForms((current) => ({ ...current, [provider]: { clientId: "", clientSecret: "", redirectUri: setting.redirectUri } }));
      ...
      <section className="glass rounded-2xl p-5">
        <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          <span className={clsx("w-fit rounded-full px-3 py-1 text-xs font-semibold", telegram?.status === "CONNECTED" ? "bg-teal-300 text-slate-950" : telegram?.status === "ERROR" ? "bg-rose-300 text-slate-950" : "bg-white/[0.08] text-slate-300")}>
            {telegram?.statusLabel ?? "Not configured"}
          </span>
        </div>
...
function normalizeTelegram(value: Partial<TelegramSettingDto> | null | undefined): TelegramSettingDto {
  const status = value?.status === "CONNECTED" || value?.status === "ERROR" ? value.status : "NOT_CONFIGURED";
  return { ...emptyTelegram, ...value, botTokenMasked: value?.botTokenMasked ?? "", chatId: value?.chatId ?? "", status, statusLabel: status === "CONNECTED" ? "Connected" : status === "ERROR" ? "Error" : "Not configured" };
}

function normalizeGoogleOAuth(value: Partial<GoogleOAuthSettingDto> | null | undefined): GoogleOAuthSettingDto {
  return { ...emptyGoogleOAuth, ...value, clientIdMasked: value?.clientIdMasked ?? "", clientSecretMasked: value?.clientSecretMasked ?? "", redirectUri: value?.redirectUri ?? "", status: value?.status === "CONNECTED" ? "CONNECTED" : "NOT_CONNECTED", statusLabel: value?.status === "CONNECTED" ? "Connected" : "Not configured" };
}

function normalizeOAuthSetting(value: Partial<OAuthProviderSettingDto> | null | undefined, provider: "TIKTOK" | "META"): OAuthProviderSettingDto {
  return { ...value, provider, clientIdMasked: value?.clientIdMasked ?? "", clientSecretMasked: value?.clientSecretMasked ?? "", redirectUri: value?.redirectUri ?? "", status: value?.status === "CONNECTED" ? "CONNECTED" : "NOT_CONNECTED", statusLabel: value?.status === "CONNECTED" ? "Connected" : "Not configured" };
}

function normalizeOAuthSettings(value: unknown): OAuthProviderSettingDto[] {
  const rows = normalizeArray<Partial<OAuthProviderSettingDto>>(value);
  return (["TIKTOK", "META"] as const).map((provider) => normalizeOAuthSetting(rows.find((item) => item.provider === provider), provider));
}
