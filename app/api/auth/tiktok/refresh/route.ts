...
export async function POST(request: Request) {
  ...
  try {
    const account = await refreshSocialOAuth(body.socialAccountId, "TIKTOK");
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "TikTok refresh failed." }, { status: 400 });
  }
}
