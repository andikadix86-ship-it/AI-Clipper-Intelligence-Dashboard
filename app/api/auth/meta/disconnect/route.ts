...
export async function POST(request: Request) {
  ...
  try {
    const account = await disconnectSocialOAuth(body.socialAccountId, "META");
    return NextResponse.json({ ok: true, authStatus: account.authStatus, permissionStatus: account.permissionStatus });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Meta disconnect failed." }, { status: 400 });
  }
}
