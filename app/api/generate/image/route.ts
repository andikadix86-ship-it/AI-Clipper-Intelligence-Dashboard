...
export async function POST(request: Request) {
  ...
  console.info("[generation-debug] completed", { provider, mode: result.mode, generationType: "IMAGE", model: result.model, promptPreview: promptPreview(finalPrompt), isDummy: result.isDummy ?? result.mode === "DUMMY", jobId: job?.id, outputSource: result.outputSource, errorMessage: runtimeInfo.warning ?? result.warning });
