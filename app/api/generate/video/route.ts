...
export async function POST(request: Request) {
  ...
  const finalPrompt = buildCreativeFinalPrompt({ prompt: body.prompt, style: body.style, type: "AI_VIDEO" });
  const result = await runtimeInfo.adapter.generateVideo({ provider, mode: runtimeInfo.mode, prompt: finalPrompt, originalPrompt: body.prompt, finalPrompt, style: body.style, aspectRatio: body.aspectRatio, apiKey: runtimeInfo.apiKey });
  ...
  console.info("[generation-debug] completed", { provider, mode: result.mode, generationType: "AI_VIDEO", model: result.model, promptPreview: promptPreview(finalPrompt), isDummy: result.isDummy ?? result.mode === "DUMMY", jobId: job?.id, outputSource: result.outputSource, errorMessage: runtimeInfo.warning ?? result.warning });
